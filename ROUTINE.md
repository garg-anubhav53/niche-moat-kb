...
4. Also pick a **geographic lens** for this run and rotate it every run (by passes mod N) so a re-swept sector surfaces *different* names. **Asymmetry lives everywhere in the trustworthy world — hunt it globally, not just US/Europe.** Rotate across, and actively pursue the under-covered Eastern democracies where we've been thin:
   ...
   **DATA-REACHABILITY REALITY (v5 — corrected):** the cloud environment proxy-**BLOCKS every non-SEC data feed** — Yahoo/price (`snapshot.py`), EDINET (Japan `screen_jp.py`), AND the TWSE OpenAPI (Taiwan `screen_tw.py`) are all unreachable in production (confirmed run #50: "TWSE OpenAPI + EDINET proxy-blocked"). **Only SEC EDGAR (US) works.** Consequences: (1) **US is the ONLY market with working systematic enumeration + trustworthy primary-source fundamentals — make it the spine and give it the majority of runs;** (2) for **all non-US markets the routine is running on §2B web-search only** — the exact hope-and-search the funnel was meant to replace — so treat every non-US name as a ~-trust LEAD with a hard confidence cap (never ✓; cap C≤2), and do **not** burn full enumeration runs pretending §2A works abroad; (3) live prices are ~-trust everywhere (Yahoo blocked). **Operator fix path:** whitelist `query1.finance.yahoo.com` / `api.edinet-fsa.go.jp` / `openapi.twse.com.tw` in the env (and set `EDINET_KEY`/`OPENDART_KEY`) to unlock international — until then, run **US-primary** and treat foreign names as low-confidence leads only.
