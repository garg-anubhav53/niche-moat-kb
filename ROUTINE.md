...
**ASYMMETRY GATE (apply after scoring — the leak we just fixed).** A high /12 is not enough. Using the §3.5 price/multiples/52-wk-range, check whether the asymmetry is *available today* (METHOD.md → Asymmetry Gate): (1) entry still open — not within ~15% of 52-wk high, not at/above analyst PT/fair value, not already re-rated ~2x; (2) realistic bull ≥ ~2x; (3) upside skewed above downside; (4) a *discrete* trigger OR CORE-grade standalone quality (diffuse "cycle turns / buyback someday / analyst may notice" does NOT count). **Fail → `PARK` ("good business, no asymmetric entry — track the buy-zone"), never a live CANDIDATE.** Most culled names were good businesses that failed *this*, not the moat test — don't re-qualify them.

**CORE override (quality-without-catalyst):** If Moat + Business quality + Floor together = **6/6** (a genuinely high-quality, well-protected business) but Catalyst = 0, the name still **QUEUES** even if the total is ≤9 — flag it `QUEUED_CORE` — **provided it still passes the Asymmetry Gate (entry open, ≥2x, skewed).** We want to own great protected businesses and wait; but a franchise already at fair value / near its high is a PARK, not a CANDIDATE. Do not let a missing catalyst bury a *cheap* franchise — but do not qualify an *expensive* one.
