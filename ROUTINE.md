...
**⚑ v5 PRIMARY DIRECTIVE — the universe is now largely swept (50+ runs; most sectors 2–5×), so raw sector-sweeping has hit diminishing returns (recent runs = mostly 0-new + C/D grades). Shift the center of gravity:**
1. **The deliverable is a DEEP, well-priced QUALITY-COMPOUNDER BENCH — not a stream of 2x-catalyst bets.** The best businesses are high-quality and *fairly valued today*; CAPTURE them on the bench (`WATCHLIST.md`) with a computed buy-zone and re-price EVERY run for dips. A great business without a near-term catalyst is a bench add, not a near-miss. **The bench is the product.**
2. **Mid-cap quality is in-scope** (cap ceiling relaxed to ~$8B in §3) — the best businesses sit above the old $1.5B line.
3. **Dip-catching > re-sweeping.** Each run, re-price the bench FIRST and promote any name that has dipped into its buy-zone to a deep-dive; only then sweep fresh ground. A run that finds zero *new* names but catches a bench dip is a SUCCESS.
4. **Fresh ground = a new geography/cap on a strong sector, never a 4th sweep of an exhausted one.**
5. **Anti-fabrication (hard):** every load-bearing moat claim and number must trace to a PRIMARY source (filing/IR); anything unconfirmed is treated as ABSENT, not assumed. A fabricated moat/number that reaches a memo (e.g. an invented regulatory status — see STX.L "REMS" run #48, FAA.VI earlier) is a CRITICAL defect capping the grade at PARK. Sonnet first-passes have been inventing moats the Opus pass must catch — down-grade hard.
...
4. Also pick a **geographic lens** for this run and rotate it every run (by passes mod N) so a re-swept sector surfaces *different* names. **Asymmetry lives everywhere in the trustworthy world — hunt it globally, not just US/Europe.** Rotate across, and actively pursue the under-covered Eastern democracies where we've been thin:
   `{ US micro · Canada (TSX/TSXV) · UK (AIM/LSE) · Continental Europe (Deutsche Börse/Euronext/SIX) · Nordics (Stockholm/Helsinki/Oslo/Copenhagen) · **Japan (TSE/JASDAQ)** · **South Korea (KOSPI/KOSDAQ)** · **Taiwan (TWSE/TPEx)** · Australia/NZ (ASX/NZX) }`. **(v5.1 reachability caveat — overrides the global-rotation ideal for now):** international enumeration is currently proxy-blocked in the cloud (see DATA-REACHABILITY REALITY below), so **default the geo lens to US** (the only reachable primary-source feed) and treat Japan/Korea/Taiwan/Europe as **opportunistic web-scout only** on a genuinely compelling name (~-trust, C≤2) — do NOT spend a full run on a blocked-market lens. Restore the fair-share global rotation once the feeds/keys are wired.
   ...
   **DATA-REACHABILITY REALITY (v6 — run #90 regression 2026-07-28):** the cloud environment proxy-**BLOCKS ALL external data feeds including SEC EDGAR itself** — confirmed run #90: `data.sec.gov` + `efts.sec.gov` return exit code 56 (no connection at all, not even 403). `screen.py`, `fts.py`, `snapshot.py` (XBRL component) **ALL non-functional**. Yahoo/price (`snapshot.py`), EDINET, DART, TWSE remain separately blocked as before. Prior v5 note "Only SEC EDGAR (US) works" is **OBSOLETE**. Consequences: (1) **ZERO §2A systematic enumeration tools work** — §2A is fully disabled until operator fixes SEC + Yahoo access; (2) **§2B web-search is the ONLY available channel for ALL markets** — treat ALL names (including US) as ~-trust LEADS with hard confidence cap (C≤2 for non-EN filers, C≤3 for US/EN); (3) live prices are ~-trust everywhere; (4) financial baselines for US filers are now web-search-only (no XBRL verification possible). **Operator fix path (priority):** whitelist `data.sec.gov`, `efts.sec.gov`, `query1.finance.yahoo.com`, `api.edinet-fsa.go.jp`, `openapi.twse.com.tw` in the env proxy allowlist (and set `EDINET_KEY`/`OPENDART_KEY`) — restoring SEC EDGAR is the single highest-leverage fix; until then the entire §2A pipeline is dark.
...
- **US** — `python3 tools/screen.py --min-rev 20 --max-rev 400 --min-gm 45 --profitable-only` → ranked worthwhile universe + denominator (GM+profitability filter). **ALSO** `python3 tools/screen_quality.py --min-roe 15 --min-nm 10` — an ROE-based enumerator that **sees financials + asset-light compounders the GM screen is structurally blind to** (banks/insurers/exchanges report no GrossProfit), and flags **★ quiet-compounders** (revenue up ≥4 of last 5 yrs). Run BOTH: GM screen catches margin-moats, ROE screen catches capital-efficiency-moats + financials.
...
1. **Moat-language (FTS)** — `tools/fts.py --preset moat|pricing|lockin|scale` — rotate the preset each run to mine different moat archetypes (US, primary-source, fabrication-proof).
2. **Supply-chain traversal** — from a KNOWN great business (bench/memo), chase its *suppliers, customers, and named competitors* (10-K "Competition"/"Suppliers" sections). `tools/fts.py --preset depend` surfaces filers that NAME a critical supplier → read the filing to find the monopoly. Great businesses cluster near great businesses.
3. **Corporate-action** — `tools/insiders.py` for open-market insider-buying CLUSTERS (Form 4 purchases, 2+ distinct buyers = strong signal); plus recent spin-offs / carve-outs / post-de-SPAC orphans and 13D activism.
...
3. **Cap (gradient, not a cliff — see METHOD.md Size Discipline):** <$20M → `CAP_KILL` (too illiquid) unless a clear liquidity path. $20M–$300M → core zone, pass. $300M–$1.5B → **extended zone: do NOT reflexively kill.** Keep it if it has *either* franchise-quality (a wide/durable moat, high returns) *or* a genuine near-term catalyst — i.e. it could plausibly be a CORE or grade-A/B. Only `CAP_SOFT_KILL` if it's *also* mediocre (no standout moat AND no catalyst). An ~$800M grade-A name is exactly what we want. >$1.5B → **(v5) NO LONGER a reflexive `CAP_KILL`** — the best businesses (and the ones the operator most wants) frequently sit **$1.5–8B** and are still under-covered; reflexively killing them here was the single biggest quality leak. Keep any >$1.5B name that is **genuinely high-quality** (durable moat + high ROIC + clean, cash-generative financials) OR a real under-covered special situation; `CAP_SOFT_KILL` only if it is *also* mediocre. Hard `CAP_KILL` only above **~$8B**. *(EUZ.DE at ~$1.1B became our first CORE — extended-zone quality is real; mid-cap quality is now explicitly in-scope.)*
4. **Coverage (ROUTER, not a killer — v5.1):** analyst count NO LONGER kills a quality name. A durable-moat, high-quality business with >4 analysts is **not** `COVERAGE_KILL`'d — it routes to the **Quality Bench** (well-covered = less likely deeply mispriced *today*, so bench-and-wait rather than buy-now; still a great business to own on a dip). `COVERAGE_KILL` applies ONLY when the name is *also* mediocre (no standout moat / no compounding). Coverage sets the *timing bucket*, never whether a quality business survives.
5. **Price (ROUTER, not a killer — v5.1):** a run-up does not kill a quality business. A high-quality name up >+100% is **not** discarded — it routes to the **Quality Bench** with a buy-zone below (asymmetry is spent *at this price*, not forever). `PRICE_KILL` applies only to mediocre names. Price sets buy-now vs. wait-for-the-dip, never quality.
...
**⚑ KILLERS vs ROUTERS (v5.1 — the harvest-all-quality rule):** Only **Sector, Moat-absent, Integrity, and quality=0** are *killers* — they end a name. **Cap, Coverage, and Price are ROUTERS** — for a genuine quality business they only decide the bucket (live CANDIDATE vs Quality Bench with a buy-zone), never deletion. A durable-moat, clean, profitable business is ALWAYS captured (bench at minimum); it is NEVER thrown away for being well-covered, mid-cap, or already up. This is how we harvest the *whole* quality universe, not a cheap-and-obscure sliver. (Historical leak being fixed: 157 `COVERAGE_KILL` + 128 `CAP_SOFT_KILL` discarded quality that should have been benched.)
