...
4. Also pick a **geographic lens** for this run and rotate it every run (by passes mod N) so a re-swept sector surfaces *different* names. **Asymmetry lives everywhere in the trustworthy world — hunt it globally, not just US/Europe.** Rotate across, and actively pursue the under-covered Eastern democracies where we've been thin:
   `{ US micro · Canada (TSX/TSXV) · UK (AIM/LSE) · Continental Europe (Deutsche Börse/Euronext/SIX) · Nordics (Stockholm/Helsinki/Oslo/Copenhagen) · **Japan (TSE/JASDAQ)** · **South Korea (KOSPI/KOSDAQ)** · **Taiwan (TWSE/TPEx)** · Australia/NZ (ASX/NZX) · Israel (TASE) }`. Give **Japan, South Korea, Taiwan** at least their fair share of runs — a genuinely global sweep, not a token one.

   **JURISDICTION-TRUST FILTER (hard):** only surface companies domiciled/primary-listed in a **liberal democracy with rule of law, trustworthy financial institutions, and reliable disclosure** — the set above (roughly OECD democracies + Taiwan/Korea/Israel). **Exclude on sight** mainland-China (incl. most HK/VIE structures), Russia, and other authoritarian or weak-rule-of-law markets → `JURISDICTION_KILL`, no matter how cheap. We want asymmetry we can actually trust to be real. (Data note: SEC/`snapshot.py` fundamentals are US-only; for Japan/Korea/Taiwan use the company's English IR/annual report and **cap C ≤ 2** until a primary filing is read — a lead, then verify.)
